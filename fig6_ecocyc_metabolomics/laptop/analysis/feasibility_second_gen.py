...
if __name__ == "__main__":
    ...
    for cpd_id, cpd in pk.compounds.items():
        if "Product_of" in cpd and cpd["Generation"] == gen:
            rxn_list2.extend(cpd["Product_of"])
            total_cpds += 1

    test_rxns = list(set(rxn_list2))

    rxn_input, failed2 = feas_filter._get_inputs(test_rxns, pk)
    feas_results = get_feasibility(rxn_input)
    

    feasibility_dict2 = dict()
    for rxn_id, feas_result in feas_results.items():
        ...
        if feas_result[2] == "feasible":
            feasibility_dict2[rxn_id] = True
        elif rxn_id not in feasibility_dict2:
            feasibility_dict2[rxn_id] = False

    for rxn_id in failed2:
        if rxn_id not in feasibility_dict2:
            feasibility_dict2[rxn_id] = False
    ...
    with open("feas_dict_g2_.pk", "wb") as f:
        pickle.dump(feasibility_dict2, f)

    with open("feas_results_g2_.pk", "wb") as f:
