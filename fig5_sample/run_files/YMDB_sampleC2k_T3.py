...
def weight(T):
    return T**3
# How to represent the function in metadata
weight_representation = "T^3"
...
database = f'kms_YMDB_2ksampleC_T3'
# Message to insert into metadata
message = f"tani sample {weight_representation}: C4C9 KEGG Target C only gen {gen} and nsample {n_sample} and frac {n_frac}"
...
rule_list, coreactant_list, rule_name = metacyc_generalized(fraction_coverage=n_frac)
write_core=False
###############################################################################
...
fingerprint, similarity = None, None
...
if __name__ == "__main__":  # required for parallelization on Windows
    ...
    if tani_sample:
        taniSampleFilter = SimilaritySamplingFilter(
            sample_size=sample_size, weight=weight, fingerprint_method=fingerprint, similarity_method=similarity
        )
